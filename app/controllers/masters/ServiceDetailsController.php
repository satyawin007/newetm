...
class ServiceDetailsController extends \Controller {
	...
	public function addServiceDetails()
	{
		if (\Request::isMethod('post'))
		{
			$values = Input::all();
			$values['active'] = "Yes"; 
			$field_names = array("sourcecity"=>"sourceCity", "destinationcity"=>"destinationCity", "serviceno"=>"serviceNo", "active"=>"active");
			$fields = array();
	...
	public function editServiceDetails()
	{
		...
		if (\Request::isMethod('post'))
		{
			$field_names = array("sourcecity1"=>"sourceCity", "servicestatu1"=>"serviceStatus", "destinationcity1"=>"destinationCity", "serviceno1"=>"serviceNo", "active1"=>"active", "servicestatus1"=>"serviceStatus");
			$fields = array();
			foreach ($field_names as $key=>$val){
				if(isset($values[$key])){
					$fields[$val] = $values[$key];
	...
	public function manageServiceDetails()
	{
		...
		$theads = array('Source City','Destination City', "Service no", "status", "service status", "Actions"); 
